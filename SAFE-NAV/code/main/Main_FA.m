function  [OUT,ref,ti] = Main_FA(OPT)
...
reward=1;
error_Q=10^-5;
STATE = linspace(0,100,1000);
ACTION = linspace(0,10,1000);
Q_mat=randn(length(STATE),length(ACTION));
R2D = (180/pi);
D2R = (pi/180);
NOS = length(ti);
NOS_g = length(tg);
 ...
 gdx =  find (tg >= (ti(i) - STAT.eps) & tg < (ti(i) + STAT.eps));
    if ( ~isempty(gdx) && gdx > 1)
        ...
        [KALMAN.F, KALMAN.G] = model(UPDATE, DCMbn, STAT.ab_corr, STAT.gb_corr);
            ...
            KALMAN.z = [ zv' zp' ]';
        KALMAN.xp(1:9) = 0;  
        [KALMAN,QS,A_index] = FA_estimator(KALMAN, dtg,i,ti(i),error_Q,Q_mat,STATE,ACTION);
        ...
        %Q_Learning
        pre_pos=[lat(gdx),lon(gdx)];
        ref_pos=[GPS.lat(gdx) GPS.lon(gdx)];
        error_Q=norm((pre_pos-ref_pos)*10^6,2);
        reward=1/(error_Q*QS);
        Q_mat(QS,A_index) = Q_mat(QS,A_index) + 0.5*(reward+0.3*max(Q_mat(QS,:)));
